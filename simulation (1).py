...
class Vehicle:
    ...
    def nextStep(self, env):
        # 在第41帧开始博弈
        if self.frame == 41 and self.state == self.PREDFINED:
            self.state = self.LANE_CHANGE
            self.target_lane_id = 1 if self.lane_id == 0 else 0#新加
        if self.state == self.PREDFINED:
            ...
            if self.side_lag_vehicle != None and self.side_lag_vehicle.side_lead_vehicle == self:
                if METHOD == 'game_theory':
                    ...
                    # 检查是否完成变道或需要取消变道
                    if self.shouldFinishLaneChange() or self.frame >= N_FRAME:
                        # 完成变道或达到帧数限制，退出博弈
                        self.state = self.LANE_KEEP
                        self.target_lane_id = None
                    else:
                        # 继续博弈
                        self.next_state = self.LANE_CHANGE#新加
    ...
    def shouldFinishLaneChange(self):
            # 检查是否完成变道
            if self.target_lane_id == 1 and self.loc_x >= env.lanes[1]._in:
                return True
            elif self.target_lane_id == 0 and self.loc_x <= env.lanes[0]._in:
                return True
            return False  # 新加
    def update(self):
